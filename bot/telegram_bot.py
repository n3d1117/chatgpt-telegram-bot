...
class ChatGPTTelegramBot:
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.edited_message or not update.message or update.message.via_bot:
            query = f"INSERT INTO history_message (msg, status) VALUES ('{prompt}', 6);"
            db.query_update(query, None)
            return
        if not await self.check_time_delay(update, context):
            query = f"INSERT INTO history_message (msg, status) VALUES ('{prompt}', 2);"
            db.query_update(query, None)
            return
        if not await self.message_censor(update, context):
            query = f"INSERT INTO history_message (msg, status) VALUES ('{prompt}', 4);"
            db.query_update(query, None)
            return
        if not await self.check_allowed(update, context):
            query = f"INSERT INTO history_message (msg, status) VALUES ('{prompt}', 3);"
            db.query_update(query, None)
            return


        self.last_message[chat_id] = prompt
        query = f"INSERT INTO history_message (msg, status) VALUES ('{prompt}', 1);"
        db.query_update(query, None)
            ...
            if self.config['stream']:
                ...
                async def _reply():
                    ...
                    if not await self.message_censor(update, context, responce_for_check):
                        query = f"INSERT INTO history_message (msg, status) VALUES ('{prompt}', 5);"
                        db.query_update(query, None)
                        return
